...
def find_nearest_exon(gff_db, gene_feature, coordinate, search_direction=None):
    ...
    """
    assert search_direction in {None, 'upstream',
                                'downstream'}, "search_direction must be None or 'upstream' or'downstream'"
    # find all gene exons - save as list of intervals
    gene_exons = [Interval(exon.start, exon.end, exon) for exon in
                  gff_db.children(gene_feature, featuretype='exon', order_by='start')]
    ...
    # find nearest
    if (search_direction == 'downstream' and gene_exons[-1].end < coordinate) or (
            search_direction == 'upstream' and gene_exons[0].end > coordinate):
        return None
    ...
    nearest_exon = IntervalTree(gene_exons)[
                   nearest_point - 1:nearest_point + 1].pop().data  # +/- 1 to avoid issues with including interval end
    return nearest_exon
...
def correct_exon(exon_feature, chrom_seq, max_coord=None, min_coord=None):
    ...
    """
    assert (max_coord or min_coord) and not (
                max_coord and min_coord), "Must provide max_coord or min_coord, but not both"
...
def correct_chimeric_gene(gene_id, chimeric_genes, gff_db, mapping_regions, genome_seq_dict):
    ...
    for gap in gap_regions:
        ...
        if gene_feature.strand == '+':
            ...
            exon_after_gap_corrected = correct_exon(exon_after_gap, chrom_seq,
                                                    max_coord=exon_before_gap_corrected.start)
...
def main():
    ...
    for chimera in chimeric_genes.keys():
        chimera_split_new_features, chimera_split_remove_features = correct_chimeric_gene(chimera, chimeric_genes,
                                                                                          gff_db, mapping_regions,
                                                                                          chrom_seq)
